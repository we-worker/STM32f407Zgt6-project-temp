...
uint16_t ADC1_Value[ADC_BUFFER_SIZE];
uint16_t ADC2_Value[ADC_BUFFER_SIZE];
uint16_t ADC3_Value[ADC_BUFFER_SIZE];
// 初始化ADC
void ADC1_Init(void)
{
...
void ADC3_Init2(void)
{
	...
	// DMA2_Stream0通道0配置
	DMA_DeInit(DMA2_Stream1); // 复位DMA2 Stream0
	...
	DMA_InitStructure.DMA_Memory0BaseAddr = (u32)&ADC3_Value;					// 内存地址
	DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralToMemory;						// 外设到内存模式
	DMA_InitStructure.DMA_BufferSize = ADC_BUFFER_SIZE;							// 数据传输量
	DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;			// 外设非增量模式
	DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;						// 内存增量模式
	...
	// 先初始化ADC1通道6和7 IO口
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3;		 // PA6 PA7
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AN;	 // 模拟输入
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL; // 不带上下拉
	GPIO_Init(GPIOC, &GPIO_InitStructure);			 // 初始化
...
{
	if (DMA_GetITStatus(DMA2_Stream1, DMA_IT_TCIF0) != RESET) // 传输完成中断
	{
		DMA_ClearFlag(DMA2_Stream1, DMA_FLAG_TCIF0);	   // 清除传输完成标志
		DMA_ClearITPendingBit(DMA2_Stream1, DMA_IT_TCIF0); // 清除中断标志
...
/////////////////////////////////////////////////////////////////////////
	...
	// 先初始化ADC1通道6和7 IO口
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1 ; // PA6 PA7
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AN;		   // 模拟输入
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;	   // 不带上下拉
	GPIO_Init(GPIOA, &GPIO_InitStructure);				   // 初始化
